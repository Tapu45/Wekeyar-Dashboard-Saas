...
export const uploadExcelFile = async (req: Request, res: Response): Promise<void> => {
  try {
    const tenantId = (req as any).user?.tenantId; // Extract tenantId from the logged-in user
    ...
    const worker = new Worker(workerPath, {
      workerData: { fileUrl: (cloudinaryResult as any).secure_url,tenantId }, // Pass the Cloudinary URL
    });
...
//     // Process the data
//     const processedData = await processData(data, req);
...
// async function processData(data: any[], req: Request) {
...
//         if (!tenantId) {
//           throw new Error("Tenant ID is required to create a store.");
//         }
...
//             tenant: {
//               connect: {
//                 id: tenantId, // Replace with the actual tenant ID
//               },
//             },
//           },
//         });
